      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-10">
            <Reveal>
              <div className="relative rounded-2xl p-10 bg-gradient-to-br from-meadow to-white border border-growth-green/15 overflow-hidden hover:shadow-xl transition-shadow">
                <div className="absolute -top-10 -right-10 w-32 h-32 rounded-full bg-growth-green/5" />
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-14 h-14 rounded-xl bg-growth-green flex items-center justify-center"><i className="fa-solid fa-seedling text-white text-xl" /></div>
                  <div>
            ...
            <Reveal delay={0.15}>
              <div className="relative rounded-2xl p-10 bg-gradient-to-br from-deep-green/[0.03] to-white border border-deep-green/12 overflow-hidden hover:shadow-xl transition-shadow">
                <div className="absolute -top-10 -right-10 w-32 h-32 rounded-full bg-harvest/5" />
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-14 h-14 rounded-xl bg-deep-green flex items-center justify-center"><i className="fa-solid fa-flask text-white text-xl" /></div>
                  <div>
                    <h3 className="font-display font-bold text-xl text-deep-green">East West Agro Chemical</h3>
                    <span className="text-xs font-semibold uppercase tracking-wider text-deep-green bg-deep-green/10 px-2.5 py-0.5 rounded-full mt-1 inline-block">Crop Protection</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  {[{ icon: 'fa-shield-halved', t: 'Crop Protection Formulas' }, { icon: 'fa-leaf', t: 'Bio-Stimulants' }, { icon: 'fa-vial', t: 'Soil Micronutrients' }, { icon: 'fa-chart-line', t: 'IPM Protocols' }].map(f => (
                    <div key={f.t} className="flex items-center gap-2 bg-deep-green/[0.05] rounded-lg px-3 py-2">
                      <i className={`fa-solid ${f.icon} text-deep-green text-xs`} />
                      <span className="text-xs font-semibold text-deep-green">{f.t}</span>
                    </div>
                  ))}
                </div>
                <Link to="/companies/agro-chemical" className="inline-flex items-center gap-2 px-6 py-2.5 bg-deep-green text-white font-semibold text-sm rounded-xl shadow-md hover:bg-[#166534] hover:-translate-y-0.5 transition-all">
